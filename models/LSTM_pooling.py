...
class LSTMClassifier(nn.Module):
	def __init__(self, batch_size, output_size, hidden_size, vocab_size, embedding_length, weights):
		...
		self.label = nn.Linear(1*hidden_size, output_size)
# 		self.label = nn.Linear(2*hidden_size, output_size)
	...
	def forward(self, input_sentence, batch_size=None):
		...
		# last h, max pooling (0 dim is seq_len = num_sequences)
# 		print("final_hidden_state", final_hidden_state.size())
		hidden_max_pooling = torch.max(output, 0)[0]
# 		print("hidden_max_pooling",hidden_max_pooling.size())
# 		hidden_mean_pooling = torch.mean(output, 0) # batch_size * hidden_size
# 		pooling_concat = torch.cat((hidden_max_pooling, hidden_mean_pooling), 1) # batch_size * 2*hidden_size
		final_output = self.label(hidden_max_pooling) 
# 		final_output = self.label(pooling_concat) 
